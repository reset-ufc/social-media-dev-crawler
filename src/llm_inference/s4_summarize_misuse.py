...
INPUT_FILE = Path(os.path.join(os.path.dirname(__file__), "misuse_cases_codes.json"))
OUTPUT_FILE = Path(os.path.join(os.path.dirname(__file__), "misuse_summary.csv"))
...
def summarize_misuse(input_path, output_path):
    ...
    with pd.ExcelWriter(output_path.with_suffix(".xlsx"), engine="openpyxl") as writer:
        ...
        df_types.to_excel(writer, sheet_name="misuse_types", index=False)
        df_categories.to_excel(writer, sheet_name="categories", index=False)
        df_subtypes.to_excel(writer, sheet_name="subtypes", index=False)
        df_with_misuse.to_excel(writer, sheet_name="posts_with_misuse", index=False)

    # CSV separado com os posts
...
if __name__ == "__main__":
    summarize_misuse(INPUT_FILE, OUTPUT_FILE)
