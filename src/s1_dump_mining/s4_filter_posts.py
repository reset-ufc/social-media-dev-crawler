...
def filter_popular_posts(input_csv=CONNECTED_POSTS, output_csv=FILTRED_POSTS, percentile=0.75):
    ...
    if null_owner_count > 0:
        ...
        if 'site_alias' in df.columns:
            null_by_site = df[null_owner_mask].groupby(
                'site_alias').size().sort_values(ascending=False)
    ...
    # All posts related to questions with null owner_id
    ...
    if not posts_to_skip.empty:
        skip_by_site = posts_to_skip.groupby(
            ['site_alias', 'type']).size().unstack(fill_value=0)

        logger.info("\nPosts to SKIP by site (will remain in dataset):")
        for site in skip_by_site.index:
            q_count = int(
                skip_by_site.loc[site, 'question']) if 'question' in skip_by_site.columns else 0
            a_count = int(
                skip_by_site.loc[site, 'answer']) if 'answer' in skip_by_site.columns else 0
            c_count = int(
                skip_by_site.loc[site, 'comment']) if 'comment' in skip_by_site.columns else 0
            total = q_count + a_count + c_count

            logger.info(
                f"  {site}: {total} posts (questions: {q_count}, answers: {a_count}, comments: {c_count})")
    else:
        logger.info("No posts to skip")

    # STEP 2: Identifying questions to consider for analysis
    logger.info(f"\n{'='*80}")
    logger.info(
        "STEP 2: Identifying questions to CONSIDER for self-answer check")
    logger.info("-" * 80)
    ...
    # All posts related to questions that will be considered
    posts_to_consider = df[df['question_id'].isin(questions_to_consider_ids)]

    logger.info(
        f"Questions with valid owner_id and answers: {len(questions_to_consider)}")
    logger.info("These questions will be checked for self-answers")

    if not posts_to_consider.empty:
        consider_by_site = posts_to_consider.groupby(
            ['site_alias', 'type']).size().unstack(fill_value=0)

        logger.info("\nPosts to CONSIDER by site:")
        for site in consider_by_site.index:
            q_count = int(consider_by_site.loc[site, 'question']
                          ) if 'question' in consider_by_site.columns else 0
            a_count = int(
                consider_by_site.loc[site, 'answer']) if 'answer' in consider_by_site.columns else 0
            c_count = int(
                consider_by_site.loc[site, 'comment']) if 'comment' in consider_by_site.columns else 0
            total = q_count + a_count + c_count

            logger.info(
                f"  {site}: {total} posts (questions: {q_count}, answers: {a_count}, comments: {c_count})")

    # STEP 3: Analyzing self-answers
    if not questions_to_consider.empty:
        ...
        # Only answers from the questions we are considering
        answers_to_check = answers[answers['question_id'].isin(
            questions_to_consider_ids)]
        ...
        if not answers_to_check.empty:
            ...
            # Check if it is a self-answer (both owner_id valid and equal)
            ...
            # Identify questions where ALL answers are self-answers
            ...
            if self_answered_question_ids:
                logger.info(
                    f"Found {len(self_answered_question_ids)} questions where ALL answers are self-answers")
                ...
                for site, row in self_answered_by_site.iterrows():
                    example_ids = all_self_answered[all_self_answered['site_alias'] == site]['question_id'].head(
                        3).tolist()
                    example_str = ", ".join(map(str, example_ids))

                    logger.info(
                        f"  {site}: {int(row['num_questions'])} questions, {int(row['num_self_answers'])} self-answers")
                    logger.info(f"    Example question IDs: [{example_str}]")

                # STEP 4: Removal
                logger.info(f"\n{'='*80}")
                logger.info(
                    "STEP 4: Removing self-answered questions and associated posts")
                logger.info("-" * 80)

                posts_before_removal = len(df)
                posts_to_remove = df[df['question_id'].isin(
                    self_answered_question_ids)]

                removal_by_site = posts_to_remove.groupby(
                    ['site_alias', 'type']).size().unstack(fill_value=0)

                logger.info("Posts REMOVED by site:")
                for site in removal_by_site.index:
                    q_count = int(
                        removal_by_site.loc[site, 'question']) if 'question' in removal_by_site.columns else 0
                    a_count = int(
                        removal_by_site.loc[site, 'answer']) if 'answer' in removal_by_site.columns else 0
                    c_count = int(
                        removal_by_site.loc[site, 'comment']) if 'comment' in removal_by_site.columns else 0
                    total = q_count + a_count + c_count
                ...
                # STEP 5: Final result after self-answer filtering
                logger.info(f"\n{'='*80}")
                logger.info("STEP 5: Final result after self-answer filtering")
                logger.info("-" * 80)

                final_by_site = df.groupby(
                    ['site_alias', 'type']).size().unstack(fill_value=0)

                logger.info("Posts REMAINING by site:")
                for site in final_by_site.index:
                    q_count = int(
                        final_by_site.loc[site, 'question']) if 'question' in final_by_site.columns else 0
                    a_count = int(
                        final_by_site.loc[site, 'answer']) if 'answer' in final_by_site.columns else 0
                    c_count = int(
                        final_by_site.loc[site, 'comment']) if 'comment' in final_by_site.columns else 0
                    total = q_count + a_count + c_count
                ...
                logger.info(
                    "All considered questions have at least one answer from another user")
        else:
    ...
    for site in sites:
        ...
        logger.info(
            f"  comment_count >= {site_quantiles['comment_count']:.2f}")
        ...
        mask = (
            ...
            (site_questions['comment_count'] >=
             site_quantiles['comment_count'])
        )
    ...
    all_popular_questions = pd.concat(
        popular_questions_list, ignore_index=True)

    logger.info(f"\n{'='*80}")
    logger.info(f"FILTERING SUMMARY")
    logger.info(f"{'='*80}")
    logger.info(
        f"Total popular questions (all sites): {len(all_popular_questions)}")
    ...
    site_type_counts = popular_related.groupby(
        ['site_alias', 'type']).size().unstack(fill_value=0)

    for site in site_type_counts.index:
        q_count = site_type_counts.loc[site,
                                       'question'] if 'question' in site_type_counts.columns else 0
        a_count = site_type_counts.loc[site,
                                       'answer'] if 'answer' in site_type_counts.columns else 0
        c_count = site_type_counts.loc[site,
                                       'comment'] if 'comment' in site_type_counts.columns else 0
